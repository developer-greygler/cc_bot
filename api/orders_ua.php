...
$db="INSERT INTO `orders_ua` (`mng`, `country`, `mng_id`,  `name`, `phone`, `email`, `product`, `price`,`comment`, `url`, `server`, `status`, `UTM`, `longtime`, `referer`, `delivery`, `ttn`,`geo`) 	VALUES 	('{$manager_id}','{$country}','{$telegram_manager_id}','{$data_array['bayer_name']}','{$data_array['phone']}','{$data_array['email']}','{$data_array['product']}','{$data_array['price']}','{$data_array['comment']}','{$data_array['site']}','{$server_json}','0','{$utm_json}','{$data_array['longtime']}','{$data_array['ref']}','1','','{$geo_json}')"; // По умолчанию - Новая Почта
...
if ($data_array['device']!="Desktop") $geo_text=$geo['as']; else $geo_text="{$geo['city_name']} {$geo['region_name']} {$geo['country_name']}";
  ...
  if ($data_array['site']!="") 
  {
	  $mess.="🔗 <b>Сайт</b>: ".stripslashes(htmlspecialchars($data_array['site']))."\n";
	  $push_order.="<li><b>Сайт</b>: ".stripslashes(htmlspecialchars($data_array['site']))."</li>";
...
$results = mysql_query("UPDATE `orders_ua` SET `tlg_id`='{$info['result']['message_id']}' {$db2} WHERE `id_num`={$myrow['id_num']}");


	 $push['message'] = $push_order."</ul>";
  $push['title'] = "Новая заявка:";
 // $push['idu_i']=$idu_i;
 // $push['idu_j']=$idu_j;
  $push['alert']=true;
